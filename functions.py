...
def get_debug_channel_id(guild_id=None) -> int | None:
    """
    Returns the debug channel ID to use.
    - If guild_id is given, checks for a server-specific override first.
    - Falls back to the global debug channel, then the hardcoded constant.
    """
    stats = load_stats()
    if guild_id:
        server_ch = stats.get("debug_channels", {}).get(str(guild_id))
        if server_ch:
            return int(server_ch)
    global_ch = stats.get("debug_channel_global")
    if global_ch:
        return int(global_ch)
    return DEBUG_CHANNEL_ID

def set_debug_channel(channel_id: int, guild_id=None):
    """
    Saves the debug channel to MongoDB.
    - guild_id=None  → sets the global debug channel
    - guild_id given → sets a server-specific override
    """
    stats = load_stats()
    if guild_id:
...
async def get_debug_channel(bot, guild_id=None):
    """Returns the debug channel object for the given guild (or global fallback)."""
    ch_id = get_debug_channel_id(guild_id)
    if ch_id is None:
        return None
...
async def send_debug_msg(bot, message: str, guild_id=None):
    if not is_debug_mode():
        return
    ch = await get_debug_channel(bot, guild_id)
    if ch is None:
        return
    try:
