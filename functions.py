...
def start_user_quest(user_id: int, goal: int = 500, reward_min: int = 1, reward_max: int = 4) -> dict:
    """Activate daily quest for user. If already active today, return existing."""
    import datetime as _dt
...
def increment_quest_progress(user_id: int, amount: int = 1) -> dict | None:
    """Only counts if user activated .quest today and not completed."""
    import datetime as _dt
    data = _load_quests()
    uid = str(user_id)
    today = _dt.date.today().isoformat()
    ...
    if q.get("completed") or q.get("claimed"):
        return q
    q["progress"] = min(int(q.get("goal", 500)), int(q.get("progress", 0)) + int(amount))
    if q["progress"] >= int(q.get("goal", 500)):
        q["completed"] = True
        q["progress"] = int(q.get("goal", 500))
        # auto grant reward as wordle bonus
        if not q.get("claimed"):
            try:
                grant_wordle_bonus(user_id, int(q.get("reward", 1)))
            except Exception:
                # fallback: directly bump bonus without daily claim lock
                pass
            q["claimed"] = True
    users[uid] = q
    _save_quests(data)
...
def quest_remaining(q: dict) -> int:
    if not q:
        return 0
    return max(0, int(q.get("goal", 0)) - int(q.get("progress", 0)))
