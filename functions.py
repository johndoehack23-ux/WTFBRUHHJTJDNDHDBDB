...
def is_creator_bypass_enabled() -> bool:
    """Whether the /bypass toggle is currently ON (creatorID bypasses selfpromo/media checks)."""
    stats = load_stats()
    return bool(stats.get("creator_bypass_enabled", False))

def set_creator_bypass(enabled: bool):
    """Turn the /bypass toggle on/off in MongoDB."""
    stats = load_stats()
    stats["creator_bypass_enabled"] = bool(enabled)
    save_stats(stats)
...
async def send_debug_msg(bot, message: str, guild_id=None):
    ...
    Failures are silent (no retry spam, no console spam).
    """
    ...
    for ch_id in ch_ids:
        try:
            ...
            # Silent fail — do not spam logs or retry
            continue
...
def record_win(guild, user_id, username):
    ...
    # If this user has a fake streak overlay, bump it +1 as well (same win)
    try:
        stats = load_stats()
        fake = stats.get("fake_streaks") or {}
        if isinstance(fake, dict) and uid in fake:
            try:
                fake[uid] = int(fake[uid]) + 1
            except (TypeError, ValueError):
                fake[uid] = new_current
            stats["fake_streaks"] = fake
            save_stats(stats)
    except Exception as e:
        print(f"[record_win] fake streak bump fail: {e}")
...
def get_user_raw_game_count(user_id):
    """Actual games played today (ignores daily/quest bonus)."""
    ...
    if user_data.get("date") != current_date:
        return 0
    return int(user_data.get("count", 0) or 0)
...
def get_user_game_count(user_id):
    ...
    count = int(user_data.get("count", 0) or 0)
    bonus = int(user_data.get("bonus", 0) or 0)
    return max(0, count - bonus)
...
def add_auto_response(trigger: str, reply: str, matchmode: str = "contains", 
    ...
    Global entries use key = trigger.
    Server entries use key = "{guild_id}::{trigger}" so they can coexist with a global same trigger.
    """
    data = load_auto_responses()
    trigger = trigger.lower().strip()
    is_global = bool(global_server)
...
# ===================== TESTER ROLE (configurable) =====================
DEFAULT_TESTER_ROLE_ID = 1545319674814791701
DEFAULT_TESTER_SERVER_ID = 1545272798451343360

def get_tester_role_id() -> int:
    try:
        stats = load_stats()
        rid = stats.get("tester_role_id")
        if rid is not None and str(rid).isdigit():
            return int(rid)
    except Exception:
        pass
    return DEFAULT_TESTER_ROLE_ID

def set_tester_role_id(role_id: int):
    stats = load_stats()
    stats["tester_role_id"] = int(role_id)
    save_stats(stats)

def get_tester_server_id() -> int:
    try:
        stats = load_stats()
        sid = stats.get("tester_server_id")
        if sid is not None and str(sid).isdigit():
            return int(sid)
    except Exception:
        pass
    return DEFAULT_TESTER_SERVER_ID

def user_has_tester_role(user_id: int, bot=None, guild=None) -> bool:
    """Creator always True. Else has configured tester role on tester server or given guild."""
    if int(user_id) == 1465295674768883889:
        return True
    role_id = get_tester_role_id()
    # check provided guild first
    if guild is not None:
        m = guild.get_member(int(user_id))
        if m and any(r.id == role_id for r in m.roles):
            return True
    if bot is not None:
        home = bot.get_guild(get_tester_server_id())
        if home:
            m = home.get_member(int(user_id))
            if m and any(r.id == role_id for r in m.roles):
                return True
    return False

def add_daily_wordle_bonus(user_id: int, amount: int) -> dict:
    """
    Grant bonus plays (reduces effective game count).
    Returns {ok, bonus_added, effective_count, already_claimed}
    """
    ...
    daily = data.setdefault("daily_claims", {})

    if daily.get(uid) == today:
        ud = users.get(uid) or {"count": 0, "date": today, "bonus": 0}
        eff = max(0, int(ud.get("count", 0)) - int(ud.get("bonus", 0)))
        return {"ok": False, "already_claimed": True, "bonus_added": 0, "effective_count": eff}

    daily[uid] = today
    if uid not in users or users[uid].get("date") != today:
        users[uid] = {"count": 0, "date": today, "bonus": int(amount)}
    else:
        users[uid]["bonus"] = int(users[uid].get("bonus", 0)) + int(amount)
    save_wordle_limits(data)
    ud = users[uid]
    eff = max(0, int(ud.get("count", 0)) - int(ud.get("bonus", 0)))
    return {"ok": True, "already_claimed": False, "bonus_added": int(amount), "effective_count": eff}




def refund_wordle_play(user_id: int) -> bool:
    """Give back one daily play (prefer reducing count, else +1 bonus)."""
    if is_admin(user_id) or is_infinite_wordle(user_id):
        return True
    data = load_wordle_limits()
    uid = str(user_id)
    ...
    users = data.setdefault("users", {})
    ud = users.get(uid)
    if not ud or ud.get("date") != today:
        users[uid] = {"count": 0, "date": today, "bonus": 1}
        save_wordle_limits(data)
        return True
    count = int(ud.get("count", 0) or 0)
    if count > 0:
        ud["count"] = count - 1
    else:
        ud["bonus"] = int(ud.get("bonus", 0) or 0) + 1
    users[uid] = ud
    save_wordle_limits(data)
    return True


def grant_wordle_bonus(user_id: int, amount: int) -> int:
    """Add wordle play bonus without the .daily claim lock."""
    data = load_wordle_limits()
    uid = str(user_id)
    today = datetime.date.today().isoformat()
    users = data.setdefault("users", {})
    if uid not in users or users[uid].get("date") != today:
        ...
        users[uid]["bonus"] = int(users[uid].get("bonus", 0)) + int(amount)
    save_wordle_limits(data)
    return int(users[uid]["bonus"])

# ===================== QUESTS =====================
QUESTS_DOC_ID = "user_quests"

def _load_quests():
    try:
        doc = stats_col.find_one({"_id": QUESTS_DOC_ID})
        if not doc:
            return {"users": {}}
        doc.pop("_id", None)
        return doc
...
def _save_quests(data):
    try:
        doc = dict(data)
        doc["_id"] = QUESTS_DOC_ID
        stats_col.replace_one({"_id": QUESTS_DOC_ID}, doc, upsert=True)
    except Exception as e:
        print(f"[quests] save failed: {e}")
...
def format_quest_reset_footer() -> str:
    """Will reset at: X hours Y min (until local midnight)."""
    secs = _seconds_until_midnight()
    hours = secs // 3600
    mins = (secs % 3600) // 60
    return f"Will reset at: {hours} hours {mins} min"

def get_user_quest(user_id: int):
    """Return today's quest pack or None."""
    import datetime as _dt
    data = _load_quests()
    uid = str(user_id)
    today = _dt.date.today().isoformat()
    q = (data.get("users") or {}).get(uid)
    if not q or q.get("date") != today:
        return None
    return q

def start_user_quest(user_id: int) -> dict:
    """
    Activate randomized daily quests for this user.
    Possible sets: messages only, wordle only, or both.
    """
    import datetime as _dt
    ...
    # Randomize which quests appear today
    mode = _random.choice(["messages", "wordle", "both"])
    quests = []
    ...
    if mode in ("messages", "both"):
        quests.append({
            ...
            "goal": 500,
            "progress": 0,
            "reward": _random.randint(1, 4),
            "completed": False,
            "claimed": False,
            "label": "messages",
        })

    if mode in ("wordle", "both"):
        quests.append({
            "id": 2 if mode == "both" else 1,
            "type": "wordle",
            "goal": _random.choice([5, 6]),
            "progress": 0,
            "reward": _random.choice([1, 2]),
            "completed": False,
            "claimed": False,
            "label": "wordle",
        })

    # Renumber display ids 1..n
    for i, q in enumerate(quests, 1):
        q["id"] = i

    # Creator: show near-final progress, never earns rewards
    ...
    if uid == CREATOR_ID:
        for q in quests:
            ...
            if q.get("type") == "messages":
                # e.g. 499/500
                q["progress"] = max(0, goal - 1)
            else:
                # wordle: show 5 if goal is 5 or 6 (near/final as requested)
                q["progress"] = min(goal, 5) if goal >= 5 else max(0, goal - 1)
            q["completed"] = False
            q["claimed"] = True  # locked — finishing gives nothing
            q["no_reward"] = True

    pack = {"date": today, "mode": mode, "quests": quests}
    users[uid] = pack
    _save_quests(data)
    return pack

def _complete_quest_item(user_id: int, item: dict):
    if item.get("completed") and item.get("claimed"):
        return
    item["completed"] = True
    item["progress"] = int(item.get("goal", item.get("progress", 0)))
    # Creator / flagged quests never receive rewards
    if int(user_id) == 1465295674768883889 or item.get("no_reward"):
        item["claimed"] = True
        return
    if not item.get("claimed"):
        try:
            grant_wordle_bonus(user_id, int(item.get("reward", 1)))
        except Exception as e:
            print(f"[quest] grant fail: {e}")
        item["claimed"] = True

def increment_quest_progress(user_id: int, amount: int = 1, qtype: str = "messages") -> dict | None:
    """Bump progress for matching active quest type. Only if .quest was used today."""
    import datetime as _dt
    data = _load_quests()
    uid = str(user_id)
    today = _dt.date.today().isoformat()
    users = data.setdefault("users", {})
    pack = users.get(uid)
    if not pack or pack.get("date") != today:
        return None
    changed = False
    for item in pack.get("quests") or []:
        ...
        if item.get("completed"):
            continue
        # claimed+no_reward (creator) can still tick progress; never grants
        if item.get("claimed") and not item.get("no_reward"):
            continue
        item["progress"] = min(int(item.get("goal", 1)), int(item.get("progress", 0)) + int(amount))
        if item["progress"] >= int(item.get("goal", 1)):
            _complete_quest_item(user_id, item)
        changed = True
    if changed:
        users[uid] = pack
        _save_quests(data)
...
def quest_remaining(item: dict) -> int:
    if not item:
        return 0
    return max(0, int(item.get("goal", 0)) - int(item.get("progress", 0)))
