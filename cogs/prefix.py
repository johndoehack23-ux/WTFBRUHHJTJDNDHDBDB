...
class PrefixCog(commands.Cog):
    ...
    async def prefix_set(self, ctx, new_prefix: str = None, scope: str = "server"):
        ...
        if scope == "global" and not bot_admin:
            return await ctx.send(get_response(F, "no_global_perm"))
        ...
        if scope == "global":
            ...
            await ctx.send(get_response(F, "changed_global", old=old_prefix, new=new_prefix))
            ...
            save_stats(stats_data)
            await ctx.send(get_response(F, "changed_server", old=old_prefix, new=new_prefix))
