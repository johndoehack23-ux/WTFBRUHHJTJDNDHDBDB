...
class HintCog(commands.Cog):
    ...
    async def hint(self, ctx):
        ...
        if not g:
            return await ctx.send(get_response(F, "no_game"))

        avail = [i for i in range(g["length"]) if i not in g.get("revealed_indices", [])]
        if not avail:
            return await ctx.send(get_response(F, "no_hints"))

        idx = random.choice(avail)
        g.setdefault("revealed_indices", []).append(idx)
        letter = g['secret'][idx].upper()
        await ctx.send(get_response(F, "hint_msg", idx=idx+1, letter=letter))
