...
class NameCog(commands.Cog):
    ...
    async def nick(self, ctx, *, args: str = None):
        ...
        # Discord nickname / display name limit is 32 characters
        ...
        # ── scope == "global" ──────────────────────────────────────────────
        # 1) Change actual bot username + global display name
        # 2) Change nickname in every server the bot is in

        account_msg = ""
        if text is not None:
            try:
                # username = login handle, global_name = display name shown everywhere
                await self.bot.user.edit(username=text, global_name=text)
                account_msg = f"✅ Bot **username** & **display name** set to **{text}**\n"
            except discord.HTTPException as e:
                # Username may be taken / rate-limited / invalid chars — still try display name alone
                try:
            ...
            # Reset display name (username cannot be "reset" to empty)
            try:
