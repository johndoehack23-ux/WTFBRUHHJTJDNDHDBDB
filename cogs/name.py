...
class NameCog(commands.Cog):
    ...
    async def nick(self, ctx, *, args: str = None):
        ...
        if text is not None and len(text) > 32:
            return await ctx.send(get_response(F, "too_long"))

        if scope == "server":
            try:
                await ctx.guild.me.edit(nick=text)
                if text is None:
                    await ctx.send(get_response(F, "reset_done"))
                else:
                    await ctx.send(get_response(F, "set_done", name=text))
            except discord.Forbidden:
                await ctx.send(get_response(F, "no_perm_nick"))
