...
class DifficultyCog(commands.Cog):
    ...
    async def difficulty_prefix(self, ctx, mode: str = "default"):
        ...
        if mode not in valid:
            return await ctx.send(get_response(F, "invalid_mode"))
        ...
        await ctx.send(get_response(F, "mode_set", mode=mode.upper()))
    ...
    async def difficulty_slash(self, interaction: discord.Interaction, mode: str):
        ...
        valid = ["easy", "medium", "hard", "impossible", "default"]
        if mode not in valid:
            return await interaction.response.send_message(get_response(F, "invalid_mode"), ephemeral=True)
        ...
        await interaction.response.send_message(get_response(F, "mode_set", mode=mode.upper()), ephemeral=True)
        await send_debug_msg(
            self.bot,
