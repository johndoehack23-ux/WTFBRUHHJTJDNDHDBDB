...
def _get_fake_map(viewer_id: int) -> dict:
    from functions import load_stats
    stats = load_stats()
    return dict((stats.get("fake_streaks") or {}).get(str(viewer_id)) or {})


def _save_fake_map(viewer_id: int, data: dict):
    from functions import load_stats, save_stats
    stats = load_stats()
    if "fake_streaks" not in stats or not isinstance(stats["fake_streaks"], dict):
        stats["fake_streaks"] = {}
    stats["fake_streaks"][str(viewer_id)] = {str(k): int(v) for k, v in data.items()}
    save_stats(stats)
...
class FakeStreakSetModal(discord.ui.Modal, title="Set Fake Streaks"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Fake streaks set for **you only** ({len(ids)} user(s)). Others will not see them.",
            ephemeral=True,
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for uid in ids:
            if uid in data:
                ...
                removed += 1
        _save_fake_map(interaction.user.id, data)
        await interaction.response.send_message(f"Removed {removed} fake streak(s) for you.", ephemeral=True)
...
class FakeStreakView(discord.ui.View):
    ...
    async def list_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("**Your fake streaks:**\n" + "\n".join(lines), ephemeral=True)
    ...
    async def removeall_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("All your fake streak data removed.", ephemeral=True)
...
class PanelView(discord.ui.View):
    ...
    @discord.ui.button(label="😂 Fake Streak", style=discord.ButtonStyle.secondary, custom_id="panel_fake_streak")
    async def fake_streak_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await self._owner_only(interaction):
            return
        embed = discord.Embed(
            title="Fake Streak 🤩",
