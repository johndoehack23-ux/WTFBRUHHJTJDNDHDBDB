...
class MoveRolesServerModal(discord.ui.Modal, title="Move Roles"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        me = guild.me
        bot_pos = me.top_role.position if me else -1
        # highest position first
        roles = sorted([r for r in guild.roles if not r.is_default()], key=lambda r: r.position, reverse=True)
        lines = []
        for r in roles:
            mark = "🔐" if r.position < bot_pos else "🔓"
            # bot's own top role still shown; roles at/above bot = unlocked for bot move
            if me and r.id == me.top_role.id:
                mark = "🤖"
        ...
        embed.set_footer(text=f"{guild.name} · `{guild.id}` · 🔐 bot above · 🔓 bot not above")
        await interaction.response.send_message(
            embed=embed,
...
class Jailbreak1Modal(discord.ui.Modal, title="Jailbreak 1"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Do not move the bot role itself
        bot_role_ids = {r.id for r in me.roles if not r.is_default()}
        if role1.id in bot_role_ids or role1.is_bot_managed() or role1.managed:
            return await interaction.response.send_message("Cannot move a bot role (roleID1).", ephemeral=True)
        if role2.is_bot_managed() and role2.id in bot_role_ids:
            pass  # still can use as anchor
        ...
        # Compute new position for role1 relative to role2
        # Discord: higher position number = higher in hierarchy
        target_pos = role2.position + 1 if pos == "above" else max(1, role2.position - 1)
        # Cannot place at/above bot top role
        if target_pos >= me.top_role.position:
            return await interaction.response.send_message(
                "Cannot place that high (at or above the bot top role).", ephemeral=True
            )
        try:
            await guild.edit_role_positions({role1: target_pos})
        ...
        await interaction.response.send_message(
            f"Moved **{role1.name}** (`{role1.id}`) **{pos}** **{role2.name}** (`{role2.id}`){give_msg}.",
            ephemeral=True,
        )
