...
WORDLE_IDLE_SECONDS = 10 * 60  # 10 minutes


class WordleCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.idle_timeout_loop.start()

    def cog_unload(self):
        self.idle_timeout_loop.cancel()

    @tasks.loop(seconds=30)
    async def idle_timeout_loop(self):
        now = time.time()
        for key, game in list(active_games.items()):
            if not isinstance(game, dict):
                continue
            if game.get("practice") or game.get("processing_win"):
                continue
            last = float(game.get("last_activity") or 0)
            if last <= 0:
                game["last_activity"] = now
                continue
            if now - last < WORDLE_IDLE_SECONDS:
                continue
            # End idle game
            active_games.pop(key, None)
            author_id = game.get("author_id")
            if author_id:
                try:
                    refund_wordle_play(int(author_id))
                except Exception as e:
                    print(f"[wordle idle] refund fail: {e}")
            # notify channel
            ch_id = game.get("channel_id") or game.get("guess_channel_id")
            try:
                ch_id = int(key) if ch_id is None and str(key).isdigit() else int(ch_id)
            except Exception:
                ch_id = None
            ch = self.bot.get_channel(ch_id) if ch_id else None
            if ch:
                try:
                    await ch.send(r("wordle", "idle_end"))
                except Exception as e:
                    print(f"[wordle idle] notify fail: {e}")
            # clean debug secret msg if any
            debug_msg_id = game.get("debug_msg_id")
            debug_ch_id = game.get("debug_msg_channel_id")
            if debug_msg_id and debug_ch_id:
                try:
                    dch = self.bot.get_channel(int(debug_ch_id))
                    if dch:
                        dm = await dch.fetch_message(int(debug_msg_id))
                        await dm.delete()
                except Exception:
                    pass

    @idle_timeout_loop.before_loop
    async def before_idle_timeout(self):
    ...
    async def _start_game(
        ...
        *,
        ...
        active_games[game_key] = {
            ...
            "channel_id": int(game_key) if str(game_key).isdigit() else None,
        }

        if not practice:
            increment_user_game_count(user.id)
            try:
                from functions import increment_quest_progress
                increment_quest_progress(user.id, 1, qtype="wordle")
            except Exception:
                pass
