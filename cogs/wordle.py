...
class WordleCog(commands.Cog):
    ...
    def _can_start_game(self, user, guild, *, practice=False):
        ...
        if count >= daily_limit:
            return False, (
                f"You have reached the maximum limit of {daily_limit} Wordle games today.\n\n"
    ...
    async def _start_game(
        ...
        *,
        ...
        if game_key in active_games or str(game_key) in active_games:
            msg = "A game is already running in that channel!"
        ...
        content = f"## New Wordle{practice_label}{mode_label} by <@{user.id}>\nLength: {len(secret_word)}"
    ...
    async def wordle_prefix(self, ctx, mode_or_option: str = None, category: str = None, difficulty: str = "easy"):
        ...
        if mode_or_option and mode_or_option.lower().strip() == "mode":
            ...
            return await ctx.send("😭🙏  Use `.wordle mode <category> <difficulty>` or just `.wordle`")
    ...
    @app_commands.describe(
        ...
        channel="Channel to start the wordle in (optional).",
        category="Category from wordlecategories.json (e.g. meme, country, animals)",
        difficulty="Difficulty for the category (easy / hard / extreme / impossible)",
    ...
    async def wordle_slash(
        ...
        if is_maintenance_mode() and not is_admin(interaction.user.id):
            return await interaction.response.send_message(
                "Bot is under maintenance.", ephemeral=True
            )

        preferred = channel if channel else interaction.channel
        ...
        # ── No custom word → normal / category start ──
        mode_label = ""
        secret_word = None
        ...
        if category:
            ...
            if not (is_user_admin or is_user_op or is_user_trusted or is_guild_admin):
                return await interaction.response.send_message(
                    "You do not have permission to use category mode.", ephemeral=True
                )
        ...
        ok = await self._start_game(
            ...
            practice=practice,
            mode_label=mode_label,
            force_preferred_channel=bool(channel),
            ctx_or_interaction=interaction,
            respond_ephemeral=True,
        )
