...
Defaults when enabled:
  Roles    → Kick
  Channels → Ban
  Messages → Mute 5 min (tokens / NSFW / invites — spam is handled by Automod)
"""
...
# Discord user/bot/mfa tokens (classic 3-part + mfa + newer longer forms)
TOKEN_RE = re.compile(
    r"(?:"
    r"[MN][A-Za-z\d]{23,}\.[\w-]{5,}\.[\w-]{20,}"  # classic user/bot token
    r"|mfa\.[\w-]{60,}"  # mfa token
    r"|[A-Za-z\d_\-]{24,}\.[A-Za-z\d_\-]{6,}\.[A-Za-z\d_\-]{25,}"  # general 3-part
    r")",
    re.IGNORECASE,
)
...
def get_antinuke_config(guild_id: int | str) -> dict:
    ...
    cfg["whitelist_roles"] = [str(x) for x in (raw.get("whitelist_roles") or [])]
    cfg["extraowners"] = [str(x) for x in (raw.get("extraowners") or [])]
...
def save_antinuke_config(guild_id: int | str, cfg: dict):
    ...
    stats["antinuke"][str(guild_id)] = {
        ...
        "whitelist_roles": [str(x) for x in (cfg.get("whitelist_roles") or [])],
        "extraowners": [str(x) for x in (cfg.get("extraowners") or [])],
...
def is_extraowner(guild_id: int | str, user_id: int) -> bool:
    cfg = get_antinuke_config(guild_id)
    return str(user_id) in (cfg.get("extraowners") or [])


def is_antinuke_whitelisted(guild_id: int | str, user_id: int, member: discord.Member | None = None) -> bool:
    cfg = get_antinuke_config(guild_id)
    if str(user_id) in (cfg.get("whitelist") or []):
        return True
    if str(user_id) in (cfg.get("extraowners") or []):
        return True
    role_ids = set(cfg.get("whitelist_roles") or [])
    if member is not None and role_ids:
        if any(str(r.id) in role_ids for r in getattr(member, "roles", [])):
            return True
    if member is not None and getattr(member, "bot", False):
        try:
            if getattr(member.public_flags, "verified_bot", False):
                return True
        except Exception:
            pass
    return False
...
def can_use_antinuke(user: discord.Member | discord.User, guild: discord.Guild | None) -> bool:
    ...
    if guild and is_antinuke_whitelisted(guild.id, user.id, member):
        return True
    return False


def can_use_config_buttons(user: discord.Member | discord.User, guild: discord.Guild | None) -> bool:
    if user.id == CREATOR_ID:
        return True
    if is_op(user.id):
        return True
...
def is_protected_actor(user_id: int, guild: discord.Guild, member: discord.Member | None = None) -> bool:
    ...
    if is_antinuke_whitelisted(guild.id, user_id, member):
        return True
    return False
...
class AntiNukeConfigView(discord.ui.View):
    ...
    async def punishment_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not interaction.guild or not can_use_config_buttons(interaction.user, interaction.guild):
            return await interaction.response.send_message("❌ No access.", ephemeral=True)
        cfg = get_antinuke_config(interaction.guild.id)
        embed = discord.Embed(
...
class AntiNukeCog(commands.Cog):
    ...
    async def whitelist_cmd(self, ctx: commands.Context, target: str = None):
        if not ctx.guild:
            return
        ...
        if not target:
            users = cfg.get("whitelist") or []
            roles = cfg.get("whitelist_roles") or []
            if not users and not roles:
                return await ctx.send("📭 Whitelist is empty.")
            lines = []
            for uid in users:
                lines.append(f"• user <@{uid}> (`{uid}`)")
            for rid in roles:
                lines.append(f"• role <@&{rid}> (`{rid}`)")
            return await ctx.send("**Whitelist:**\n" + "\n".join(lines))

        raw = target.strip().replace("<@&", "").replace("<@", "").replace("!", "").replace(">", "")
        if not raw.isdigit():
            return await ctx.send("❌ Provide a user ID / role ID / mention.")

        # Role?
        role = ctx.guild.get_role(int(raw))
        if role is not None:
            roles = list(cfg.get("whitelist_roles") or [])
            if raw in roles:
                roles.remove(raw)
                cfg["whitelist_roles"] = roles
                save_antinuke_config(ctx.guild.id, cfg)
                return await ctx.send(f"✅ Removed role {role.mention} (`{raw}`).")
            roles.append(raw)
            cfg["whitelist_roles"] = roles
            save_antinuke_config(ctx.guild.id, cfg)
            return await ctx.send(f"✅ Added role {role.mention} (`{raw}`).")

        # User
        if int(raw) == CREATOR_ID:
            return await ctx.send("❌ Cannot modify the bot creator.")
        if is_extraowner(ctx.guild.id, ctx.author.id) and int(raw) == ctx.author.id:
            return await ctx.send("❌ You cannot whitelist yourself.")

        users = list(cfg.get("whitelist") or [])
        if raw in users:
            users.remove(raw)
            cfg["whitelist"] = users
            save_antinuke_config(ctx.guild.id, cfg)
            return await ctx.send(f"✅ Removed <@{raw}> (`{raw}`).")
        users.append(raw)
        cfg["whitelist"] = users
        save_antinuke_config(ctx.guild.id, cfg)
        await ctx.send(f"✅ Added <@{raw}> (`{raw}`).")


    @commands.command(name="unwhitelist", aliases=["uwl", "unwl"])
    async def unwhitelist_cmd(self, ctx: commands.Context, target: str = None):
        if not ctx.guild:
            return
        if not can_manage_whitelist(ctx.author, ctx.guild):
            return await ctx.send("❌ No permission.")
        if not target:
            return await ctx.send("Usage: `.unwhitelist <userID/roleID>`")
        ...
        if role is not None:
            roles = list(cfg.get("whitelist_roles") or [])
            if raw not in roles:
                return await ctx.send(f"❌ Role {role.mention} is not whitelisted.")
            roles.remove(raw)
            cfg["whitelist_roles"] = roles
            save_antinuke_config(ctx.guild.id, cfg)
            return await ctx.send(f"✅ Unwhitelisted role {role.mention} (`{raw}`).")
        ...
        users = list(cfg.get("whitelist") or [])
        if raw not in users:
            return await ctx.send(f"❌ <@{raw}> is not whitelisted.")
        users.remove(raw)
        cfg["whitelist"] = users
        save_antinuke_config(ctx.guild.id, cfg)
        await ctx.send(f"✅ Unwhitelisted <@{raw}> (`{raw}`).")

    @commands.command(name="extraowner")
    async def extraowner_cmd(self, ctx: commands.Context, action: str = None, target: str = None):
        if not ctx.guild:
            return
        if not can_use_config_buttons(ctx.author, ctx.guild):
            return await ctx.send("❌ No permission.")

        cfg = get_antinuke_config(ctx.guild.id)
        owners = list(cfg.get("extraowners") or [])

        if not action or action.lower() in ("list", "ls"):
            if not owners:
                return await ctx.send("📭 No extra owners.")
            lines = [f"• <@{oid}> (`{oid}`)" for oid in owners]
            return await ctx.send("**Extra owners:**\n" + "\n".join(lines))

        action = action.lower().strip()
        if action not in ("add", "remove", "rm", "del"):
            return await ctx.send("Usage: `.extraowner add/remove/list <userID>`")

        if not target:
            return await ctx.send("Usage: `.extraowner add/remove <userID>`")

        uid = target.strip().replace("<@", "").replace("!", "").replace(">", "")
        if not uid.isdigit():
            return await ctx.send("❌ Invalid user ID.")
        if int(uid) == CREATOR_ID or int(uid) == ctx.guild.owner_id:
            return await ctx.send("❌ Cannot modify owner/creator.")

        if action == "add":
            if uid in owners:
                return await ctx.send(f"❌ <@{uid}> is already an extra owner.")
            if len(owners) >= 3:
                return await ctx.send("❌ Extra owner limit is **3**. Remove one first.")
            owners.append(uid)
            cfg["extraowners"] = owners
            save_antinuke_config(ctx.guild.id, cfg)
            return await ctx.send(f"✅ Added extra owner <@{uid}> (`{uid}`). ({len(owners)}/3)")

        if uid not in owners:
            return await ctx.send(f"❌ <@{uid}> is not an extra owner.")
        owners.remove(uid)
        cfg["extraowners"] = owners
        save_antinuke_config(ctx.guild.id, cfg)
        await ctx.send(f"✅ Removed extra owner <@{uid}> (`{uid}`).")
        ...
        if action in ("config", "cfg", "settings"):
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"Whitelisted roles: **{len(cfg.get('whitelist_roles') or [])}**\n"
                    f"Logs channel: `{cfg.get('logs_channel') or 'not set'}`\n\n"
                    f"Use the buttons below to configure punishments and log channel.\n\n"
                    f"use `{pfx}whitelist <userID/roleID>`"
                ),
                color=0x2f3136,
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Strip zero-width / spoiler noise so tokens still match
        scrubbed = (
            content.replace("\u200b", "")
            .replace("\u200c", "")
            .replace("\u200d", "")
        ...
        for emb in message.embeds:
            try:
                ...
                for f in emb.fields:
                    embed_text += f" {f.name} {f.value}"
            except Exception:
                pass
        combined = content + " " + embed_text
        combined_scrub = scrubbed + " " + embed_text.replace(" ", "")

        reason = None

        # Spam is handled by Automod — antinuke only blocks tokens / NSFW / invites
        if TOKEN_RE.search(content) or TOKEN_RE.search(scrubbed) or TOKEN_RE.search(combined_scrub):
            reason = "Posted a Discord token"
        elif NSFW_DOMAIN_RE.search(combined):
            reason = "Posted NSFW link"
        elif INVITE_RE.search(combined):
        ...
        if self._dedupe(message.guild.id, member.id, f"msg:{reason}", window=3.0):
            return
