...
Defaults when enabled:
  Roles    → Kick
  Channels → Ban
  Messages → Mute 5 min (spam: 5 msgs / 3s)
"""
...
DEFAULT_SPAM = {"count": 5, "window": 3.0}  # 5 msgs within 3s
...
class AntiNukeCog(commands.Cog):
    ...
    async def antinuke_cmd(self, ctx: commands.Context, action: str = None, *rest):
        ...
        if action in ("enable", "on", "true"):
            ...
            await ctx.send(
                "✅ **Anti Nuke ENABLED**\n"
                "Defaults: **Roles → Kick** · **Channels → Ban** · **Messages → Mute 5m** (5 msgs / 3s)"
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Spam: always 5 messages within 3 seconds (hard requirement)
        if reason is None:
            limit = 5
            window = 3.0
            ...
            while q and (now - q[0]) > window:
                q.popleft()
            if len(q) >= limit:
                reason = f"Spam ({len(q)} messages in {window}s)"
                q.clear()
                print(
                    f"[antinuke] SPAM detected guild={message.guild.id} user={member.id} "
        ...
        # Only de-dupe identical spam bursts briefly so we still mute once
        if self._dedupe(message.guild.id, member.id, "msg:spam" if reason.startswith("Spam") else f"msg:{reason}", window=2.0):
            return
