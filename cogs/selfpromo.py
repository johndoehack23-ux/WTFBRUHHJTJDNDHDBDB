...
def _is_media_exempt(member: discord.Member) -> bool:
    """Never delete: creator, server owner, extraowner."""
    ...
    try:
        ...
        stats = load_stats()
        eo = ((stats.get("antinuke") or {}).get(str(member.guild.id)) or {}).get("extraowners") or []
        if str(member.id) in [str(x) for x in eo]:
            return True
    except Exception:
        pass
    return False
...
class SelfPromoCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if _is_media_exempt(member):
            return
        if is_admin(member.id, message.guild) or is_op(member.id):
            return

        guild_id  = str(message.guild.id)
        channel_id = str(message.channel.id)
        channels  = get_selfpromo_channels(guild_id)
        channels = {str(k): v for k, v in (channels or {}).items()}
        ...
        if mode == MODE_INVITE:
            valid = bool(INVITE_PATTERN.search(message.content or ""))
            warn_text = "Only Discord server invite links are allowed in this channel."
        elif mode == MODE_LINKS:
            valid = bool(LINKS_PATTERN.search(message.content or ""))
            warn_text = "Only platform links (YouTube, TikTok, Twitch, etc.) are allowed in this channel."
        else:
            valid = bool(INVITE_PATTERN.search(message.content or "")) or bool(LINKS_PATTERN.search(message.content or ""))
    ...
    @commands.command(name="discontinuedslashcommand1")
    async def discontinuedslashcommand1(self, ctx: commands.Context, channel: discord.TextChannel = None):
        """Prefix replacement for old /fixpromo — wipe self-promo Mongo data for this server."""
        if not ctx.guild:
            return
        if not _has_permission(ctx.author):
        ...
        if channel:
            ...
            if removed:
                await ctx.send(f"✅ Removed {channel.mention} from self-promo data in MongoDB.")
            else:
                await ctx.send(f"⚠️ {channel.mention} was not in self-promo data — nothing to remove.")
        else:
            count = fix_selfpromo_guild(guild_id)
            await ctx.send(f"✅ Wiped all self-promo data for this server from MongoDB ({count} channel(s) removed).")

        await send_debug_msg(
            self.bot,
            f"🔧 `.discontinuedslashcommand1` | {ctx.author} (`{ctx.author.id}`) wiped selfpromo data"
            + (f" for {channel.name}" if channel else " (all)") + f" | {ctx.guild.name}",
            guild_id=guild_id,
        )
