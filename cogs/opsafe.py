...
def get_cfg(guild_id) -> dict:
    ...
    join_window = max(3600, min(10 * 86400, join_window))  # 1 hour .. 10 days
    return {
...
def parse_join_timer(text: str) -> int | None:
    """Parse '1 Day', '2 days', '1 hour', '3 hours'. Minutes not allowed. Max 10 days."""
    ...
    if unit in ("minute", "min", "m"):
        return None  # minutes not allowed
    if unit in ("hour", "hr", "h"):
        ...
        return None
    if secs < 3600 or secs > 10 * 86400:
        return None
    return secs


class OpSafeJoinModal(discord.ui.Modal, title="Edit Join Value"):
    max_join = discord.ui.TextInput(
        label="Maximum limit of joining users",
        placeholder="1-15 (default 6)",
        required=True,
        max_length=2,
    )
    timer = discord.ui.TextInput(
        label="Timer of how many users join in that time",
        placeholder="e.g. 1 Day, 1 hour (max 10 days)",
        required=True,
        max_length=20,
    )
    wip3 = discord.ui.TextInput(
        label="WIP",
        placeholder="WIP",
        required=False,
        max_length=10,
    )
    wip4 = discord.ui.TextInput(
        label="WIP",
        placeholder="WIP",
        required=False,
        max_length=10,
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if secs is None:
            return await interaction.response.send_message(
                "Invalid timer. Use `1 Day`, `2 days`, `1 hour` (max 10 days). Minutes not allowed.",
                ephemeral=True,
            )
        ...
        days = secs / 86400
        if days >= 1:
            tlabel = f"{days:g} day(s)"
        else:
            tlabel = f"{secs / 3600:g} hour(s)"
        await interaction.response.send_message(
            f"Join values updated for this server: limit **{limit}** / window **{tlabel}**",
            ephemeral=True,
        )
...
class OpSafeConfigView(discord.ui.View):
    ...
    @discord.ui.button(label="Edit Join Value", style=discord.ButtonStyle.success, custom_id="opsafe_edit_join")
...
class OpSafeCog(commands.Cog):
    ...
    async def opsafe_cmd(self, ctx: commands.Context, action: str = None):
        ...
        if action == "test":
            if ctx.author.id != CREATOR_ID:
                return await ctx.send("No permission.")
            # Safe pre-check — never locks channels (creator only, hidden)
            lines = []
            me = ctx.guild.me
            cfg = get_cfg(ctx.guild.id)

            lines.append(f"Status stored: **{'ON' if cfg.get('enabled') else 'OFF'}** · locked={cfg.get('locked')}")

            # Mongo write test (toggle a harmless field)
            try:
                probe = get_cfg(ctx.guild.id)
                probe["_test"] = True  # not persisted field name in save — use enabled readback
                save_cfg(ctx.guild.id, {
                    "enabled": probe.get("enabled", False),
                    "locked": probe.get("locked", False),
                    "saved_overwrites": probe.get("saved_overwrites") or {},
                })
                again = get_cfg(ctx.guild.id)
                lines.append("MongoDB read/write: **OK**")
            except Exception as e:
                lines.append(f"MongoDB: **FAIL** ({e})")

            if me is None:
                lines.append("Bot member: **missing cache**")
            else:
                perms = me.guild_permissions
                checks = [
                    ("Administrator (or Manage Channels)", perms.administrator or perms.manage_channels),
                    ("Ban Members", perms.ban_members),
                    ("View Audit Log", perms.view_audit_log),
                    ("Manage Roles", perms.administrator or perms.manage_roles),
                ]
                ...
                if roles:
                    ...
                    high = me.top_role.position >= top.position - 1
                    lines.append(f"Bot role near top: **{'OK' if high else 'LOW — move higher'}** ({me.top_role.name})")

            bits = get_antinuke_bits(ctx.guild.id)
            logs = bits.get("logs_channel")
            lines.append(f"Logs channel: **{('#' + str(logs)) if logs else 'not set (optional)'}**")

            lines.append("")
            lines.append("**What enable does:** only starts monitoring. **No channels are changed.**")
            lines.append("**Lockdown only runs if:** join flood (config limit/window) **and** a new member spam (6 msgs / 2s).")
            lines.append("Use `.opsafe unlock` only after a real lockdown.")

            return await ctx.send("**OpSafe pre-test**\n" + "\n".join(lines))
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # Join flood alone does NOT lockdown — needs spam signal too (within last 2 min)
        if len(joins) >= limit and not cfg.get("locked"):
