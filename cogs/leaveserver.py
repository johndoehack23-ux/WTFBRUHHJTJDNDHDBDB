    ...
    def _update_buttons(self):
        ...
        async def prev_cb(interaction: discord.Interaction, v=self):
            if interaction.user.id != v.author_id:
                return await interaction.response.send_message(get_response("leaveserver", "not_your_list"), ephemeral=True)
...
class LeaveConfirmationView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.ctx.author.id:
            await interaction.response.send_message(get_response("leaveserver", "not_your_confirm"), ephemeral=True)
            return False
        return True
...
class LeaveServerCog(commands.Cog):
    ...
    async def leave_prefix(self, ctx, target: str = None, action: str = None):
        ...
        if target_clean == "server" and action_clean == "list":
            ...
            if not entries:
                return await ctx.send(get_response("leaveserver", "no_history"))
        ...
        if not target_clean.isdigit():
            return await ctx.send(get_response("leaveserver", "invalid_id"))

        guild_id = int(target_clean)

        if guild_id == self.MAIN_SERVER_ID:
            return await ctx.send(get_response("leaveserver", "fire"))

        guild = self.bot.get_guild(guild_id)
        if not guild:
            return await ctx.send(get_response("leaveserver", "not_in_server"))
    ...
    async def execute_leave_single(self, ctx, guild):
        try:
            ...
            await ctx.send(get_response("leaveserver", "leave_failed", error=e))
