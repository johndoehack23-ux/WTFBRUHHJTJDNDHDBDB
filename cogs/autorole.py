...
class AutoRoleCog(commands.Cog):
    ...
    autorole_group = app_commands.Group(
        name="autorole",
        description="Configure autorole for members and bots",
        guild_only=True,
    )

    @autorole_group.command(name="set", description="Set member/bot roles and apply to everyone already in the server")
    @app_commands.describe(
        member_role="Role for human members",
        bots="Whether bots also get a role",
        bot_role="Role for bots (needed if bots=True)",
        enabled="Enable autorole after saving (default True)",
    )
    ...
    @autorole_group.command(name="toggle", description="Turn autorole on or off")
    ...
    @autorole_group.command(name="status", description="Show current autorole settings")
...
async def setup(bot: commands.Bot):
    ...
    # Ensure group is on the tree (discord.py usually auto-adds from cog)
    try:
        bot.tree.add_command(cog.autorole_group)
    except app_commands.CommandAlreadyRegistered:
        pass
    print("[autorole] cog loaded — /autorole set|toggle|status")
