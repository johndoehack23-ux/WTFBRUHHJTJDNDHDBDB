...
class AutoRoleCog(commands.Cog):
    ...
    @app_commands.describe(
        ...
        bots="true = also give bots a role",
        bot_role="Role for bots (required if bots=true)",
        enabled="Turn autorole on/off (default: true)",
    )
    ...
    async def autorole(
        ...
        enabled: Optional[app_commands.Choice[str]] = None,
    ):
        ...
        if on:
            m_ok, b_ok, fail = await backfill_roles(
                interaction.guild,
                member_role,
                bot_role if bots_on else None,
                bots_on,
            )
            try:
                await interaction.followup.send(
                    f"Backfill done · members: **{m_ok}** · bots: **{b_ok}** · failed: **{fail}**",
                    ephemeral=True,
                )
            except Exception:
                pass
