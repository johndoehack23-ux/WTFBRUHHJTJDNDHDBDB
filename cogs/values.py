...
class ValuesCog(commands.Cog):
    ...
    @commands.command(name="getvalues")
    async def getvalues_prefix(self, ctx):
        if not _owner_only(ctx.author):
        ...
        try:
            ...
            return await ctx.send("❌ The JSON export failed.")

    @commands.command(name="importvalues")
    async def importvalues_prefix(self, ctx):
        if not _owner_only(ctx.author):
            return await ctx.send("❌ Only the bot owner can use this command.")
        if not ctx.message.attachments:
            return await ctx.send("❌ Upload a `.zip` file containing the JSON files.")
        archive = ctx.message.attachments[0]
        ...
        try:
            ...
            if not imported:
                return await ctx.send("❌ The ZIP did not contain any JSON files.")
            with tempfile.TemporaryDirectory() as temp_dir:
                staged = Path(temp_dir)
                for filename, raw in imported.items():
                    (staged / filename).write_bytes(raw)
                for filename in imported:
                    (Path(".") / filename).write_bytes((staged / filename).read_bytes())
            _refresh_loaded_json_globals()
        except (discord.HTTPException, zipfile.BadZipFile, UnicodeDecodeError, json.JSONDecodeError, ValueError, OSError) as error:
            print(f"[importvalues] {error}")
            return await ctx.send("❌ Import failed. Upload a valid ZIP containing valid JSON files with unique filenames.")
        await ctx.send(f"✅ Imported `{len(imported)}` JSON file(s) successfully.")
