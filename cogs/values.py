...
class ValuesCog(commands.Cog):
    ...
    async def getvalues(self, interaction: discord.Interaction):
        ...
        try:
            ...
            return await interaction.response.send_message(
                "❌ I could not DM you. Please enable DMs from this server, then try again.",
                ephemeral=True,
            )
        except (discord.HTTPException, OSError, zipfile.BadZipFile) as error:
            print(f"[getvalues] {error}")
            return await interaction.response.send_message(
                "❌ The JSON export failed.",
                ephemeral=True,
            )

        await interaction.response.send_message(
            f"✅ Sent `{len(included)}` JSON file(s) to your DMs.",
            ephemeral=True,
        )
    ...
    async def importvalues(
        ...
        if not archive.filename.lower().endswith(".zip"):
            return await interaction.response.send_message(
                "❌ Upload a `.zip` file containing JSON files.",
                ephemeral=True,
            )
        ...
        try:
            ...
            return await interaction.response.send_message(
                "❌ Import failed. Upload a valid ZIP containing valid JSON files with unique filenames.",
                ephemeral=True,
            )

        if not imported:
            return await interaction.response.send_message(
                "❌ The ZIP did not contain any JSON files.",
                ephemeral=True,
            )
        ...
        try:
            ...
            print(f"[importvalues] {error}")
            return await interaction.response.send_message(
                "❌ The JSON files could not be written to the project.",
                ephemeral=True,
            )

        await interaction.response.send_message(
            f"✅ Imported `{len(imported)}` JSON file(s) into the project.",
            ephemeral=True,
        )
