...
def get_server_trusted_users(guild_id: str) -> list:
    """Fetch trusted users for a specific guild from MongoDB (via load_stats)."""
    stats = load_stats()
    return stats.get("trusted_users", {}).get(str(guild_id), [])

def build_server_autoresponder_embed(data: dict, guild_id: str, guild_name: str) -> discord.Embed:
    """Build the current server autoresponder list, including global responders."""
    local_responders = []
    global_responders = []
    ...
    for trigger, content in data.items():
        ...
        elif str(content.get("guild_id", "")) == str(guild_id):
            local_responders.append(f"• `{trigger}`")

    embed = discord.Embed(title="🤖 **Current Server Autoresponders**", color=0x2f3136)
    embed.set_footer(text=f"Server: {guild_name}")
    embed.add_field(
        name="🏡 Local Server Responders",
        value="\n".join(local_responders) if local_responders else "No local server responders set.",
        inline=False,
    )
    embed.add_field(
        name="🌐 Global Responders",
...
class AutoresponderCog(commands.Cog):
    ...
    @app_commands.choices(action=[
        ...
        app_commands.Choice(name="removeall", value="removeall"),
    ])
    @app_commands.describe(
        action="add | edit | list | removeall",
    ...
    async def autoresponder(
        ...
        if action == "list":
            embed = build_server_autoresponder_embed(
                get_all_auto_responses(),
                guild_id,
        ...
        if action == "add":
            ...
            existing = get_all_auto_responses()

            # Adding GLOBAL but a global entry already exists for this trigger
            if is_global:
                for _k, entry in existing.items():
                    if entry.get("trigger", "").lower().strip() == trigger_key and entry.get("global"):
                        return await interaction.response.send_message(
                            "❗Theres already a global autoresponder for that! Please use the edit mode",
                            ephemeral=True,
                        )

            # Adding SERVER: allowed even if global exists — stores server override (does not wipe global)
            if react:
            ...
            global_label = " 🌐 [GLOBAL]" if is_global else " (server override)"
    ...
    async def deleteautoresponder(
        ...
        if is_maintenance_mode() and not is_bot_admin:
            return await interaction.response.send_message(r("maintenance", "on"), ephemeral=True)
    ...
    async def showresponders(self, ctx, scope: str = "server"):
        ...
        if scope == "global":
            ...
            embed = build_server_autoresponder_embed(data, guild_id, ctx.guild.name)
