...
class AutoresponderCog(commands.Cog):
    ...
    async def autoresponder(
        ...
        if not is_bot_admin and not is_server_trusted and not interaction.permissions.administrator:
            return await interaction.response.send_message(get_response(F, "no_permission"), ephemeral=True)
        ...
        if action == "add":
            ...
            await interaction.response.send_message(get_response(F, "added_global" if is_global else "added", trigger=trigger), ephemeral=True)
            ...
            await interaction.response.send_message(get_response(F, "edited", trigger=trigger), ephemeral=True)
    ...
    async def deleteautoresponder(
        ...
        if is_item_global:
            ...
            await interaction.response.send_message(get_response(F, "deleted_global", trigger=trigger), ephemeral=True)
            ...
            remove_auto_response(target_trigger)
            await interaction.response.send_message(get_response(F, "deleted_local", trigger=trigger), ephemeral=True)
