    ...
    async def mode_1v1(self, ctx, mode: str = None, arg1: str = None, arg2: str = None, arg3: str = None):
        ...
        if mode_action == "end":
            ...
            if channel_id in active_1v1_lobbies:
                ...
                await ctx.send(get_response(F, "lobby_ended"))
                ended_something = True

            if channel_id in active_1v1_matches:
                del active_1v1_matches[channel_id]
                await ctx.send(get_response(F, "match_ended"))
                ended_something = True
            ...
            if ended_something:
                ...
                await ctx.send(get_response(F, "no_active"))
            return

        if mode_action != "1v1":
            return await ctx.send(get_response(F, "no_active"))

        if channel_id in active_games or channel_id in active_1v1_lobbies or channel_id in active_1v1_matches:
            return await ctx.send(get_response(F, "already_active"))
        ...
        if arg1 and arg2:
            ...
            if uid1.isdigit() and uid2.isdigit() and len(uid1) >= 15 and len(uid2) >= 15:
                try:
                    ...
                    return await ctx.send(get_response(F, "not_found_users"))

                if member1.bot or member2.bot:
                    return await ctx.send(get_response(F, "no_bots"))
                if member1.id == member2.id:
                    return await ctx.send(get_response(F, "same_user"))
        ...
        try:
            ...
            if len(users) < 2:
                await ctx.send(get_response(F, "not_enough"))
                return
            ...
            await ctx.send(get_response(F, "error_match"))
            print(f"1v1 Error: {e}")
