...
class MediaCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if _is_media_exempt(member):
            return
        # Creator optional bypass still respected when not exempt path above (creator is exempt always)
        ...
        # Normalize keys every time (survives redeploys / type drift)
        settings = {str(k): bool(v) for k, v in (settings or {}).items()}
