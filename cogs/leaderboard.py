...
from functions import get_tester_role_id, is_admin, is_op, is_maintenance_mode, load_stats
...
def _load_fake_players_for_viewer(viewer_id, mode: str, guild_id=None) -> list:
    """Fake Player entries visible to this viewer."""
    try:
        from functions import stats_col
        ...
        return []
    out = []
    vid = str(viewer_id) if viewer_id is not None else None
    for e in raw:
        if not isinstance(e, dict):
            continue
        vis = str(e.get("visibility") or "everyone")
        if vis != "everyone" and vis != vid:
            continue
        sid = e.get("server_id")
        if mode == "server":
            if sid is None or str(sid) != str(guild_id):
                # custom name-only fake players show on global; for server need matching id
                continue
        # global: include all visible; if server_id set it still shows on global with that server name
        out.append({
            "user_id": f"fake_{e.get('username','x')}_{e.get('streak',0)}",
            "username": str(e.get("username") or "Unknown")[:30],
            "display_name": str(e.get("username") or "Unknown")[:30],
            "current_streak": int(e.get("streak") or 0),
            "server_name": str(e.get("server_name") or "Unknown"),
            "_fake_player": True,
        })
    return out


def _hecker_entries(count: int = 99999) -> list:
    # Virtual-ish: build list (may be large). Cap memory by generating only what's needed in view if needed.
    return [
        {
            "user_id": f"hecker_{i}",
            "username": "hacker",
...
def _is_tester_only(user, guild, bot=None) -> bool:
    """Tester role but not creator/op/admin — can see responses, cannot change data."""
    from functions import is_admin, is_op
    CREATOR_ID = 1465295674768883889
    TESTER_ROLE_ID = 1545319674814791701
    TESTER_SERVER_ID = 1545272798451343360
...
class PageModal(discord.ui.Modal, title="Go to Page"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if getattr(self.lb_view, "viewer_id", None) is not None and interaction.user.id != self.lb_view.viewer_id:
            return await interaction.response.send_message(
                "Only the person who ran the leaderboard command can use these buttons.",
                ephemeral=True,
            )
        try:
...
def _load_all_fake_streaks() -> dict:
    """
    Global map: target_user_id -> fake streak value.
    Only the TARGET sees this when THEY run .lb — not the person who set it, not anyone else.
    """
    try:
        ...
        # Support both shapes:
        #   new: { "targetId": 99 }
        #   old nested: { "setterId": { "targetId": 99 } }  -> flatten targets
        out = {}
        if not isinstance(raw, dict):
            return out
        for k, v in raw.items():
            if isinstance(v, dict):
                # old nested format — keep target values
...
def _apply_fake_streaks_for_viewer(entries, viewer_id):
    """
    Client-side only for the TARGET:
    If viewer_id has a fake streak assigned, only THEY see it on THEIR row.
    Setter and everyone else see the real board.
    """
    fakes = _load_all_fake_streaks()
    vid = str(viewer_id)
    if vid not in fakes:
        return list(entries)

    fake_val = int(fakes[vid])
    out = []
    found = False
    for e in entries:
        e = dict(e)
        if str(e.get("user_id", "")) == vid:
            e["current_streak"] = fake_val
            e["_fake"] = True
            found = True
        out.append(e)
    ...
    if not found:
        out.append({
            "user_id": vid,
            "username": f"User {vid}",
            "display_name": f"User {vid}",
            "current_streak": fake_val,
            "server_name": "—",
            "_fake": True,
        })

    out.sort(key=lambda x: int(x.get("current_streak", 0) or 0), reverse=True)
    return out


class LeaderboardView(discord.ui.View):
    def __init__(self, entries, mode, guild_name="", viewer_id=None):
        super().__init__(timeout=120)
        self.viewer_id = viewer_id
        if viewer_id is not None:
            entries = _apply_fake_streaks_for_viewer(list(entries), viewer_id)
        else:
        ...
        self.total_pages = min(
            ...
            max(1, math.ceil(len(entries) / ENTRIES_PER_PAGE) if entries else 1),
        )
        self.update_buttons()
    ...
    def build_embed(self):
        ...
        # Footer: rank/streak of the person who ran .lb (after their personal fake overlay)
        viewer_rank = None
        viewer_streak = 0
        if self.viewer_id is not None:
            vid = str(self.viewer_id)
            for i, e in enumerate(self.entries, 1):
                if str(e.get("user_id", "")) == vid:
                    viewer_rank = i
                    viewer_streak = int(e.get("current_streak", 0) or 0)
                    break
        if viewer_rank is not None:
            embed.set_footer(text=f"Your rank: {viewer_rank} | Streak: {viewer_streak}")
        else:
            embed.set_footer(text="Your rank: — | Streak: 0")
        ...
        for i, entry in enumerate(page_entries, start_rank + 1):
            ...
            current = int(entry.get("current_streak", 0) or 0)
            ...
            # Plain name only — no mentions, continuous rank i across pages
            # Escape backticks so a name/server like 096`1 does not break the next rows
            if str(username).startswith("<@") and str(username).endswith(">"):
            ...
            safe = _clean(username)
            if self.mode == "global":
                server_name = _clean(entry.get("server_name", "Unknown Server"))
                lines.append(f"{i}. {safe} - {current}{streak_emoji} - {server_name}")
            else:
                lines.append(f"{i}. {safe} - {current}{streak_emoji}")

        embed.description = "\n".join(lines)
        return embed

    async def _owner_only(self, interaction: discord.Interaction) -> bool:
        """Only the user who ran .lb / .lb server can use these buttons."""
    ...
    def update_buttons(self):
        ...
        for p in range(1, 11):
            ...
            async def _pcb(interaction: discord.Interaction, pg=p - 1, v=self):
                if not await v._owner_only(interaction):
                    return
                v.current_page = pg
                v.update_buttons()
        ...
        async def jump_fwd_cb(interaction: discord.Interaction, v=self):
            if not await v._owner_only(interaction):
                return
            v.current_page = min(v.total_pages - 1, v.current_page + 10)
            v.update_buttons()
            await interaction.response.edit_message(embed=v.build_embed(), view=v)
...
class LeaderboardCog(commands.Cog):
    ...
    def _display_user(self, user_id, username, guild=None):
        # Plain username only — never a Discord mention
        user = self._resolve_user_obj(user_id, guild)
        if user:
            return getattr(user, "name", None) or username or "Unknown"
    ...
    def _build_global_entries(self):
        ...
        for doc in cursor:
            ...
            if uid not in best_per_user or current > best_per_user[uid]["current_streak"]:
                uname = getattr(user, "name", None) or doc.get("username", "Unknown")
                best_per_user[uid] = {
                    "user_id": uid,
                    "username": uname,
                    "display_name": uname,  # never mention — plain name only
    ...
    async def streak_group(self, ctx):
        await ctx.send("❓ **Usage:**\n`.streak set <userID> <value> [serverID/none]`\n`.streak reset <@user> [global/server]`")

    @streak_group.command(name="set")
    async def streak_set_prefix(self, ctx, user: str, num: int, server: str = None):
        """
        .streak set <userID> <value> [serverID/none]
        serverID -> that server; none/omitted -> current server
        """
        if not _has_tester_or_admin(ctx.author, ctx.guild, ctx.bot):
            return await ctx.send("You do not have permission to use this command.")
        if _is_tester_only(ctx.author, ctx.guild, ctx.bot):
            return await ctx.send("Tester role can use `.streak` replies, but cannot change streaks.")

        uid = str(user).strip().replace("<@", "").replace("!", "").replace(">", "")
        if not uid.isdigit():
            return await ctx.send("Provide a numeric userID or mention.")

        if server is None or str(server).lower().strip() in ("none", "null", "-", "here", "this"):
            if not ctx.guild:
                return await ctx.send("No server specified and not in a server.")
            gid_str = str(ctx.guild.id)
        else:
            sid = str(server).strip()
            if not sid.isdigit():
                return await ctx.send("serverID must be numeric or `none`.")
            gid_str = sid
        ...
        username = uid_str
        try:
            u = self.bot.get_user(int(uid_str)) or await self.bot.fetch_user(int(uid_str))
            if u:
                username = u.name
        except Exception:
            pass
        ...
        self.leaderboard_col.update_one(
            ...
            {"$set": {
                ...
                "user_id": uid_str,
                "username": username,
                "current_streak": int(num)
            }},
            upsert=True
        )
        await ctx.send(
            f"✅ Updated **{username}** (`{uid_str}`) streak on server `{gid_str}`: `{old_streak}` → `{num}`"
        )

    @streak_group.command(name="reset")
    async def streak_reset_prefix(self, ctx, user: discord.Member, scope: str = "server"):
        if not _has_tester_or_admin(ctx.author, ctx.guild, ctx.bot):
            return await ctx.send("You do not have permission to use this command.")
        if _is_tester_only(ctx.author, ctx.guild, ctx.bot):
            return await ctx.send("Tester role can use `.streak` replies, but cannot change streaks.")
        ...
        if scope not in ("global", "server"):
            return await ctx.send("Usage: `.streak reset <@user> [global/server]` (default: server)")
        ...
        if scope == "global":
            ...
            await ctx.send(f"Reset streak for {user.name} globally ({result.modified_count} docs).")
        else:
            if not ctx.guild:
                return await ctx.send("Server scope needs a server.")
            gid_str = str(ctx.guild.id)
            result = self.leaderboard_col.update_one(
                {"_id": f"{gid_str}_{uid_str}"},
                {"$set": {"current_streak": 0}}
            )
            await ctx.send(f"Reset streak for {user.name} on this server.")
    ...
    async def lb(self, ctx, scope: str = "global"):
        ...
        if scope in ("money", "cash"):
            cog = ctx.bot.get_cog("MoneyLeaderboardCog")
            if cog:
                return await cog.show(ctx)
            return await ctx.send("Money leaderboard unavailable.")
        if scope not in ("global", "server"):
            return await ctx.send("❌ Usage: `.lb global` / `.lb server` / `.lb money`")

        if is_hecker_mode_active():
            # Temporary overlay — does not delete real data
            entries = _hecker_entries(99999)
            view = LeaderboardView(entries, mode="global" if scope == "global" else "server",
                                  guild_name=(ctx.guild.name if ctx.guild and scope == "server" else "HACKED"),
        ...
        if scope == "global":
            ...
            fakes = _load_fake_players_for_viewer(ctx.author.id, "global")
            entries = list(entries) + fakes
            entries.sort(key=lambda x: int(x.get("current_streak", 0) or 0), reverse=True)
            if not entries:
                return await ctx.send("🏆 No stats yet globally!")
            view = LeaderboardView(entries, mode="global", viewer_id=ctx.author.id)
            self._sync_page_cache("global", entries)
        else:
            if not ctx.guild:
                return await ctx.send("Server scope needs a server.")
            entries = self._build_server_entries(ctx.guild.id)
            fakes = _load_fake_players_for_viewer(ctx.author.id, "server", guild_id=ctx.guild.id)
            entries = list(entries) + fakes
            entries.sort(key=lambda x: int(x.get("current_streak", 0) or 0), reverse=True)
            if not entries:
                return await ctx.send("🏆 No stats yet on this server!")
            view = LeaderboardView(entries, mode="server", guild_name=ctx.guild.name, viewer_id=ctx.author.id)
            self._sync_page_cache("server", entries, guild_id=ctx.guild.id)
