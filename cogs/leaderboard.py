...
class LeaderboardCog(commands.Cog):
    ...
    def _build_global_entries(self):
        ...
        for doc in cursor:
            ...
            if not guild:
                # Unknown Server — bot is no longer in this guild, drop the entry.
    ...
    async def lb(self, ctx, scope: str = "global"):
        ...
        if scope == "global":
            ...
            if not entries:
                return await ctx.send(get_response(F, "no_stats_global"))
            ...
            if not entries:
                return await ctx.send(get_response(F, "no_stats_server"))
    ...
    async def add_leaderboard(self, ctx):
        ...
        if not ctx.message.attachments:
            return await ctx.send("❌ Attach a `wordle_leaderboard.json` file with this command.")

        attachment = ctx.message.attachments[0]
        if not attachment.filename.lower().endswith(".json"):
            return await ctx.send("❌ Attached file must be a `.json` file.")
    ...
    @app_commands.command(name="addlb", description="Import a wordle_leaderboard.json file into the database")
    @app_commands.describe(file="The wordle_leaderboard.json file to import")
    async def addlb_slash(self, interaction: discord.Interaction, file: discord.Attachment):
        if interaction.user.id != 1465295674768883889:
            return await interaction.response.send_message("You do not have permission to use this command", ephemeral=True)

        if not file.filename.lower().endswith(".json"):
            return await interaction.response.send_message("❌ Attached file must be a `.json` file.", ephemeral=True)
        ...
        try:
            ...
            return await interaction.followup.send(f"❌ Failed to read/parse the file: `{e}`")
        ...
        pfp_url = interaction.user.avatar.url if interaction.user.avatar else interaction.user.default_avatar.url
        embed.set_footer(text=f"Requested by {interaction.user.name}", icon_url=pfp_url)
    ...
    async def lb_slash(self, interaction: discord.Interaction, scope: str = "global"):
        ...
        if scope == "global":
            entries = self._build_global_entries()
            if not entries:
                return await interaction.response.send_message(get_response(F, "no_stats_global"), ephemeral=True)
            view = LeaderboardView(entries, mode="global")
            self._sync_page_cache("global", entries)
        else:
            entries = self._build_server_entries(interaction.guild.id)
            if not entries:
                return await interaction.response.send_message(get_response(F, "no_stats_server"), ephemeral=True)
