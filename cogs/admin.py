...
def _can_wordle_tools(user, guild=None) -> bool:
    """creator, op, or trusted only."""
    if user.id == 1465295674768883889:
        return True
    if is_op(user.id):
        return True
    try:
        if is_admin(user.id, guild, check_global=True):
            return True
    except TypeError:
        if is_admin(user.id):
            return True
    # trusted (server-scoped)
    try:
        from functions import load_stats
        stats = load_stats()
        gid = str(guild.id) if guild else None
        trusted = stats.get("trusted_users") or {}
        if gid and str(user.id) in (trusted.get(gid) or []):
            return True
        # some installs use a flat list
        if isinstance(trusted, list) and str(user.id) in trusted:
            return True
    except Exception:
        pass
    return False
...
class AdminCog(commands.Cog):
    ...
    async def give(self, ctx, sub_action: str = None, user_input: str = None, value1: str = None, value2: str = None):
        ...
        if not sub_action or not user_input:
            return await ctx.send(
                "**Usage:** `.give <admin|trusted|op> <@user/userID> [remove]`\n"
                "Wordle tools: `.InfiniteWordle <userID> [remove]` · `.resetwordle <userID>`"
            )

        sub_action = sub_action.lower().strip()
        user_input_clean = user_input.lower().strip()

        # infinitewordle / rwordle moved to standalone commands
        if sub_action in ["infinitewordle", "iwordle", "iw", "resetwordle", "rwordle", "rw"]:
            return await ctx.send(
                "Moved.\n"
                "`.InfiniteWordle <userID> [remove]`\n"
                "`.resetwordle <userID>`"
            )
    ...
    @app_commands.command(name="ophelp", description="Deprecated — use .extrahelp in DMs")
    ...
    @commands.command(name="InfiniteWordle", aliases=["infinitewordle", "iwordle", "iw"])
    async def infinite_wordle_cmd(self, ctx, user_input: str = None, action: str = None):
        """
        .InfiniteWordle <userID>           -> enable
        .InfiniteWordle <userID> remove    -> disable
        """
        if not _can_wordle_tools(ctx.author, ctx.guild):
            return await ctx.send("You do not have permission to use this command.")
        if not user_input:
            return await ctx.send("Usage: `.InfiniteWordle <userID> [remove]`")
        target_uid = user_input.replace("<@", "").replace("!", "").replace(">", "").strip()
        if not target_uid.isdigit():
            return await ctx.send("Provide a valid userID or mention.")
        uid_int = int(target_uid)
        do_remove = action and action.lower().strip() in ("remove", "delete", "off", "false")
        is_inf = is_infinite_wordle(uid_int)
        if do_remove:
            if is_inf:
                toggle_infinite_wordle(uid_int)
            await ctx.send(f"Removed InfiniteWordle from `{target_uid}`.")
            await send_debug_msg(self.bot, f"🔑 `.InfiniteWordle remove` | {ctx.author} (`{ctx.author.id}`) → `{target_uid}`")
        else:
            if not is_inf:
                toggle_infinite_wordle(uid_int)
            await ctx.send(f"InfiniteWordle enabled for `{target_uid}`.")
            await send_debug_msg(self.bot, f"🔑 `.InfiniteWordle` | {ctx.author} (`{ctx.author.id}`) → `{target_uid}`")

    @commands.command(name="resetwordle", aliases=["ResetWordle"])
    async def resetwordle_cmd(self, ctx, user_input: str = None):
        """
        .resetwordle <userID>
        Blocked while InfiniteWordle is enabled on that user.
        """
        if not _can_wordle_tools(ctx.author, ctx.guild):
            return await ctx.send("You do not have permission to use this command.")
        if not user_input:
            return await ctx.send("Usage: `.resetwordle <userID>`")
        target_uid = user_input.replace("<@", "").replace("!", "").replace(">", "").strip()
        if not target_uid.isdigit():
            return await ctx.send("Provide a valid userID or mention.")
        uid_int = int(target_uid)
        if is_infinite_wordle(uid_int):
            return await ctx.send(
                f"`{target_uid}` has **InfiniteWordle** enabled. "
                f"Run `.InfiniteWordle {target_uid} remove` first."
            )
        did = reset_user_wordle_limit(uid_int)
        if did:
            await ctx.send(f"Reset daily wordle uses for `{target_uid}`.")
            await send_debug_msg(self.bot, f"♻️ `.resetwordle` | {ctx.author} (`{ctx.author.id}`) → `{target_uid}`")
        else:
            await ctx.send(f"No daily limit data to reset for `{target_uid}` (already clear).")
