...
class AdminCog(commands.Cog):
    ...
    @commands.command(name="debugchannel")
    async def set_debug_channel_cmd(self, ctx, channel: discord.TextChannel = None, scope: str = "server"):
        if not (is_op(ctx.author.id) or is_admin(ctx.author.id, ctx.guild, check_global=True)):
            return await ctx.send("You do not have permission to use this command.")

        # If a channel mention is provided, force scope to server regardless of what was typed
        if channel and scope == "server":
            scope = "server"
        elif not channel:
            return await ctx.send(
                "❌ Usage: `.debugchannel <#channel> [server/global]`\n"
                "• Default scope is `server`\n"
                "• Use `global` to set the global debug channel (op only)"
            )
        ...
        set_debug_channel(channel.id, guild_id=guild_id)

        scope_label = f"server ({ctx.guild.name})" if scope == "server" else "global"
        await ctx.send(f"✅ Debug channel set to {channel.mention} for **{scope_label}**.")
        await send_debug_msg(
            self.bot,
            f"🔧 `.debugchannel` | {ctx.author} (`{ctx.author.id}`) set debug channel to {channel.mention} [{scope_label}] | {ctx.guild.name}",
