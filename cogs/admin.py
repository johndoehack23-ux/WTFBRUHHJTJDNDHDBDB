    ...
    async def give(self, ctx, sub_action: str = None, user_input: str = None, value1: str = None, value2: str = None):
        ...
        if sub_action in ["infinitewordle", "iwordle", "iw"]:
            ...
            if user_input_clean in ["rall", "dall", "removeall", "deleteall"]:
                ...
                return await ctx.send(get_response(F, "infinite_all"))
            ...
            if is_remove_action:
                ...
                await ctx.send(get_response(F, "infinite_removed", uid=target_uid))
                ...
                if not is_infinite:
                    toggle_infinite_wordle(uid_int)
                await ctx.send(get_response(F, "infinite_enabled", uid=target_uid))
        ...
        if sub_action in ["resetwordle", "rwordle", "rw"]:
            ...
            if did_reset:
                ...
                if is_remove_action:
                    await ctx.send(get_response(F, "limit_reset_full", uid=target_uid))
                else:
                    await ctx.send(get_response(F, "limit_reset", uid=target_uid))
        ...
        if sub_action in ["admin", "op"]:
            ...
            if is_remove_action:
                ...
                if target_uid in user_pool:
                    ...
                    save_stats(stats)
                    await ctx.send(get_response(F, "removed_global"))
                    ...
                    await ctx.send(get_response(F, "not_in_list", role=sub_action))
            else:
                if target_uid in user_pool:
                    user_pool.remove(target_uid)
                    save_stats(stats)
                    ...
                    save_stats(stats)
                    await ctx.send(get_response(F, "added_global", role=sub_action))
