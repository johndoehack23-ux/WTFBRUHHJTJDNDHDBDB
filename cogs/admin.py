...
class AdminCog(commands.Cog):
    ...
    async def on_command(self, ctx):
        ...
        await send_debug_msg(
            self.bot,
            f"🟦 PREFIX `{full_name}` | {ctx.author} (`{ctx.author.id}`) | #{getattr(ctx.channel, 'name', 'unknown')} | {ctx.guild.name} (`{ctx.guild.id}`) | `{content[:500]}`",
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        await send_debug_msg(
            self.bot,
            f"🟪 SLASH `/{command_name}`{option_text} | {interaction.user} (`{interaction.user.id}`) | #{getattr(interaction.channel, 'name', 'unknown')} | {interaction.guild.name} (`{interaction.guild.id}`)",
