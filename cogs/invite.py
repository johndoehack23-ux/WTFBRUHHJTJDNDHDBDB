...
class InviteCog(commands.Cog):
    ...
    async def add_invite_management(self, ctx, category: str = None, target_id: str = None, action: str = None):
        ...
        if not is_op(ctx.author.id):
            return await ctx.send(r("invite", "no_permission"))
        ...
        if category == "cleanall":
            ...
            stats["invited_users"] = []
            save_stats(stats)
            return await ctx.send(r("invite", "wiped"))
        ...
        if category == "user":
            ...
            if action and action.lower().strip() == "remove":
                if clean_id in pool:
                    ...
                    save_stats(stats)
                    return await ctx.send(r("invite", "user_removed", id=clean_id))
                return await ctx.send("❌ User not found in whitelist.")

            if clean_id in pool:
                return await ctx.send(r("invite", "user_exists"))

            pool.append(clean_id)
            save_stats(stats)
            return await ctx.send(r("invite", "user_added", id=clean_id))
                ...
                if clean_id in pool:
                    ...
                    if guild_to_leave:
                        try:
                            ...
                            pass
                    return await ctx.send(r("invite", "server_removed", id=clean_id))
                return await ctx.send("❌ Server not found in allowed list.")

            if clean_id in pool:
                return await ctx.send(r("invite", "server_exists"))

            # Append as a string to perfectly match main.py's json verification loop
            pool.append(clean_id)
            save_stats(stats)
            return await ctx.send(r("invite", "server_added", id=clean_id))
