...
def get_logs_channel_id(guild_id: int) -> int | None:
    try:
        stats = load_stats()
        am = ((stats.get("automod") or {}).get(str(guild_id)) or {}).get("logs_channel")
...
def format_config_description(cfg: dict) -> str:
    ...
    ch_txt = f"`{logs}`" if logs else "`not set`"
    return (
        f"**Spamming**\n"
        f"Mute: {mute} min\n"
        f"Limit: {limit}\n"
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            return await interaction.response.send_message(
                "Mute, Limit, and Seconds must be whole numbers.", ephemeral=True
            )
        if mute < 1 or mute > 1440:
            return await interaction.response.send_message("Mute must be 1-1440 minutes.", ephemeral=True)
        if limit < 2 or limit > 50:
            return await interaction.response.send_message("Limit must be 2-50.", ephemeral=True)
        if seconds < 1 or seconds > 30:
            return await interaction.response.send_message("Seconds must be 1-30.", ephemeral=True)
        ...
        await interaction.response.send_message(
            f"Updated: Mute **{mute}**m · Limit **{limit}** · Seconds **{seconds}**",
            ephemeral=True,
        )


class AutomodLogsModal(discord.ui.Modal, title="Set Automod Logs"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        save_automod_config(interaction.guild.id, cfg)
        await interaction.response.send_message(
            f"Automod logs set to {ch.mention} (`{cid}`)", ephemeral=True
        )
...
class AutomodConfigView(discord.ui.View):
    ...
    @discord.ui.button(label="Set logs", style=discord.ButtonStyle.secondary, custom_id="automod_cfg_logs")
    async def logs_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not interaction.guild or not can_use_automod(interaction.user, interaction.guild):
            return await interaction.response.send_message("No access.", ephemeral=True)
        await interaction.response.send_modal(AutomodLogsModal())
...
class AutoModCog(commands.Cog):
    ...
    async def automod_cmd(self, ctx: commands.Context, action: str = None, *rest):
        ...
        if action in ("enable", "on", "true"):
            # No hierarchy requirement (same as antinuke)
            me = ctx.guild.me
            missing = []
