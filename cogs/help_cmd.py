...
def build_help_embed(user_id, guild, guild_id_str):
    ...
    # Determine the color and tier label
    if is_user_op:
        color = 0x5865F2
    elif is_user_admin:
        color = 0xED4245
    ...
    if is_privileged:
        normal_lines = (
            ...
            f"`{prefix}mode 1v1` — Start a 1v1 wordle match\n"
    ...
    if is_user_trusted or is_user_admin or is_user_op:
        trusted_lines = (
        ...
        embed.add_field(name="🟡 Trusted", value=trusted_lines, inline=False)
    ...
    if is_user_admin or is_user_op:
        admin_lines = (
            ...
            f"`{prefix}hint` — Reveal a random hint letter for the current game\n"
            ...
            f"`{prefix}give iw <@user> add` — Give a user infinite wordle plays\n"
    ...
    if is_user_op:
        op_lines = (
            ...
            f"`{prefix}eg global` / `/endgame global` — End all active games across every server\n"
    ...
    if not is_privileged:
        ...
        embed.set_footer(text=f"Access level: {'Op' if is_user_op else 'Admin' if is_user_admin else 'Trusted'} • Use .wordle secrethelp for full details")
