...
def build_help_embed(user_id, guild, guild_id_str):
    ...
    if is_user_admin or is_user_op:
        admin_lines = (
            ...
            f"`{prefix}status <status> \"<activity text>\"` — Set the bot's status and activity *(owner only)*\n"
...
class HelpCog(commands.Cog):
    ...
    async def help_slash(self, interaction: discord.Interaction):
        if is_maintenance_mode() and not is_admin(interaction.user.id):
            return await interaction.response.send_message(get_response("help_cmd", "maintenance"), ephemeral=True)
