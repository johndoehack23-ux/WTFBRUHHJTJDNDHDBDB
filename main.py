...
async def setup_hook():
    ...
    # --- Slash command sync (autorole debug) ---
    try:
        local = [c.name for c in bot.tree.get_commands()]
        print(f"📋 Local tree ({len(local)}): {', '.join(sorted(local))}")
    except Exception as e:
        print(f"📋 local tree fail: {e}")

    try:
        synced = await bot.tree.sync()
        print(f"🚀 GLOBAL sync ({len(synced)}): {', '.join(sorted(c.name for c in synced))}")
    except Exception as e:
        print(f"❌ GLOBAL sync failed: {e}")

    # Clear guild-scoped copies so Discord does not show DUPLICATE slash cmds (e.g. 2x /panel)
    GUILD_IDS = (1545272798451343360,)
    for gid in GUILD_IDS:
        try:
            g = discord.Object(id=gid)
            bot.tree.clear_commands(guild=g)
            await bot.tree.sync(guild=g)  # push empty guild set — global cmds remain once
            print(f"🧹 Guild {gid} command duplicates cleared (global-only slash cmds)")
        except Exception as e:
            print(f"❌ Guild {gid} clear failed: {e}")

    # Fetch back from Discord API (source of truth)
    try:
        fetched = await bot.tree.fetch_commands()
        names = sorted(c.name for c in fetched)
        print(f"🔎 Discord API GLOBAL fetch ({len(names)}): {', '.join(names)}")
        print("✅ API has autorole" if "autorole" in names else "⚠️ API missing autorole")
    except Exception as e:
        print(f"❌ fetch_commands global failed: {e}")

    for gid in GUILD_IDS:
        try:
            g = discord.Object(id=gid)
            fetched = await bot.tree.fetch_commands(guild=g)
            names = sorted(c.name for c in fetched)
            print(f"🔎 Discord API GUILD {gid} fetch ({len(names)}): {', '.join(names)}")
            print(f"✅ Guild API has autorole" if "autorole" in names else f"⚠️ Guild API missing autorole")
        except Exception as e:
            print(f"❌ fetch_commands guild {gid} failed: {e}")
...
async def on_message(message):
    ...
    if not maintenance_block and message.guild and not is_server_blacklisted(message.guild.id):
        ...
        for game_key in possible_game_keys:
            ...
            if len(content) == game["length"] and content.isalpha() and not content.startswith("."):
                game["last_activity"] = __import__("time").time()
        ...
        # Collect matches; prefer server-local over global for the same trigger
        matches = []
        ...
        if matches:
            # local (not global) first
            matches.sort(key=lambda d: 0 if not d.get("global") else 1)
            data = matches[0]
            trigger = data.get("trigger", "").lower()
            if check_cooldown(str(message.guild.id), trigger):
