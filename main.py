...
async def setup_hook():
    ...
    # Push a guild copy so the server sees commands immediately
    ...
    for gid in GUILD_IDS:
        try:
            ...
            print(f"🚀 GUILD {gid} sync ({len(gsynced)}): {', '.join(sorted(c.name for c in gsynced))}")
        except Exception as e:
            print(f"❌ GUILD {gid} sync failed: {e}")
