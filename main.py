    ...
    try:
        ...
        if msg_id:
            try:
                ...
                print(f"⚠️ Could not edit old status message ({msg_id}): {e} — sending new one")
...
async def _set_offline_status():
    """Best-effort: edit status message to red before the process exits."""
    try:
        await update_status_message("Status: 🔴")
    except Exception as e:
        print(f"❌ Failed to set offline status message: {e}")

def _handle_shutdown(signum, frame):
    """Catch SIGTERM/SIGINT (Railway stop / Ctrl+C) and set Status: 🔴."""
    global _shutting_down
    if _shutting_down:
        return
    _shutting_down = True
    sig_name = signal.Signals(signum).name if hasattr(signal, "Signals") else str(signum)
    print(f"🛑 Received {sig_name} — setting status to 🔴 then closing bot...")
    try:
        loop = bot.loop
        if loop.is_running():
            fut = asyncio.run_coroutine_threadsafe(_set_offline_status(), loop)
            try:
                fut.result(timeout=8)  # wait briefly so the edit can finish
            except Exception as e:
                print(f"⚠️ Offline status edit timed out or failed: {e}")
            # Ask discord.py to close cleanly
            loop.call_soon_threadsafe(lambda: asyncio.create_task(bot.close()))
        else:
            # Loop not running — nothing we can do async
            print("⚠️ Event loop not running; cannot edit status message.")
    except Exception as e:
        print(f"❌ Shutdown handler error: {e}")

# Register handlers so Railway stop (SIGTERM) and Ctrl+C (SIGINT) set 🔴
