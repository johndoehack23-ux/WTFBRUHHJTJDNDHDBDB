...
async def on_guild_join(guild: discord.Guild):
    ...
    # Wait longer for any pending .addinvite writes to fully commit to MongoDB
    await asyncio.sleep(5)

    # Read fresh from MongoDB up to 3 times with short delays — handles slow writes
    allowed = []
    for attempt in range(3):
        fresh_config = load_stats()
        allowed = [str(x) for x in fresh_config.get("allowed_servers", []) if str(x).isdigit()]
        if allowed and str(guild.id) in allowed:
            # Server is whitelisted — welcome
            print(f"✅ Joined whitelisted server: {guild.name} ({guild.id})")
            return
        if allowed:
            break
        # allowed_servers came back empty — retry after short delay
        await asyncio.sleep(2)

    if not allowed:
        print(f"⚠️  on_guild_join: allowed_servers empty, skipping leave for {guild.name} ({guild.id})")
        return
            ...
            if not match.get("guessed") and len(content) == match["length"] and content.isalpha() and not content.startswith("."):
                if message.author.id == match["p1"]["id"] or message.author.id == match["p2"]["id"]:
                    ...
                    if content == secret:
                        ...
                        winner["score"] += 5
                        winner["wins"] += 1
                        await message.channel.send(get_response("wordle", "1v1_correct", feedback=feedback, winner=winner['name']))

                        if winner["wins"] >= 2:
                            await message.channel.send(get_response("wordle", "1v1_match_over", winner=winner['name'], loser=loser['name']))
                            if channel_id in active_1v1_matches:
            ...
            if len(content) == game["length"] and content.isalpha() and not content.startswith("."):
                ...
                if content == secret:
                    ...
                    if not is_practice:
                        ...
                        await message.channel.send(get_response("wordle", "correct_word", feedback=feedback, user_id=message.author.id))
                    else:
                        await message.channel.send(get_response("wordle", "correct_practice", feedback=feedback, user_id=message.author.id))
                else:
