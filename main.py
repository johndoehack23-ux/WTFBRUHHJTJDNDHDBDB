...
async def setup_hook():
    ...
    # List commands registered on the tree after loading cogs
    try:
        local = [c.name for c in bot.tree.get_commands()]
        print(f"📋 Local tree commands ({len(local)}): {', '.join(sorted(local))}")
        if "autorole" not in local:
            print("⚠️ autorole is NOT on the local command tree — cog did not register the slash command")
        else:
            print("✅ autorole is on the local command tree")
    except Exception as e:
        print(f"📋 tree list fail: {e}")

    # Global sync
    try:
        synced = await bot.tree.sync()
        names = sorted(c.name for c in synced)
        print(f"🚀 Global slash sync OK ({len(synced)}): {', '.join(names)}")
        if "autorole" not in names:
            print("⚠️ autorole missing from GLOBAL sync result")
        else:
            print("✅ autorole present in GLOBAL sync result")
    except Exception as e:
        print(f"❌ Global slash sync FAILED: {e}")

    # Copy global → guild then guild-sync so commands appear immediately in this server
    for gid in (1545272798451343360,):
        try:
            g = discord.Object(id=gid)
            bot.tree.copy_global_to(guild=g)
            gsynced = await bot.tree.sync(guild=g)
            gnames = sorted(c.name for c in gsynced)
            print(f"🚀 Guild {gid} slash sync OK ({len(gsynced)}): {', '.join(gnames)}")
            if "autorole" not in gnames:
                print(f"⚠️ autorole missing from guild {gid} sync")
            else:
                print(f"✅ autorole present in guild {gid} sync")
        except Exception as e:
            print(f"❌ Guild {gid} slash sync FAILED: {e}")
